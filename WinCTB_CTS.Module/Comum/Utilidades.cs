...
namespace WinCTB_CTS.Module.Comum {
    public static class Utilidades {
        ...
        public static IEnumerable<IList<T>> SplitQuery<T>(this IEnumerable<T> source, int parts) {
            return source
        ...
        public static IEnumerable<IList<T>> SplitSizeQuery<T>(this IEnumerable<T> source, int parts) {
            return source
                .Select((x, i) => new { Index = i, Value = x })
                .GroupBy(x => x.Index / parts)
                .Select(x => x.Select(v => v.Value).ToList())
